...
##### 3.2.14 Second-order Spatial Point Patterns Analysis
...
Here is the code to perform L function
...
![](images/L-function)1.2%20for%20non.png)
...
B <- FunctionalWaterpoint$status_group
```

```{r}
NonFunctionalWaterpoint <- wp_Osun %>%
filter(status_group == "Nonfunctional")
A <- NonFunctionalWaterpoint$status_group
```
